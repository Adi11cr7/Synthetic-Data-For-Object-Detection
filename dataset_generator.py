...
def get_mask_file(img_file):
    ...
    mask_file = os.path.join("\\".join(path[:-2]), "masks", path[-1].split(".")[0]+".jpg")
    return mask_file
...
# img_list = get_list_of_images('data_dir\objects_dir\lego\imgs')
# masks = []
# for img in img_list:
#     masks.append(get_mask_file(img))
# # print(masks)
# # print(get_labels(img_list))
# annotations = []
# for mask in masks:
#     annotations.append(get_annotation_from_mask(mask))
# print(annotations)
...
def gen_syn_data(img_files, labels, img_dir, anno_dir, scale_augment, rotation_augment, dontocclude, add_distractors):
    ...
    params_list = []
    count_loop = 0;
    print("\nlenght of imag_labels = {}\n".format(len(img_labels)))
    # while len(img_labels) > 0:
    # while len(background_files) > count_loop:
    for k in range(len(background_files)):
        count_loop += 1
        # Get list of objects
        objects = []
        ...
        for i in range(n):
            ...
            objects.append(img_labels[random.randint(0,len(img_labels)-1)])
